...
class GameStateTest extends AnyFlatSpec with Matchers { 
  ...
  "towersAttack" should "attack enemies in range of towers" in {
    val game = new TowerDefenceGame(3)
    val state = new GameState(game)
    val pos1 = GridPos(9,2)
    val pos2 = GridPos(3,2)
    val pos3 = GridPos(10,2)
    val pos4 = GridPos(4,2)
    val pos5 = GridPos(30,30)
    ...
    state.towersAttack()
    tanker1.health shouldBe comparisonTanker.health - basic1.damage 
    tanker2.health shouldBe comparisonTanker.health - splashTower.damage 
  }
